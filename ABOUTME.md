<p align="left">
>> aboutme.mď
</p>

<div align="center">
  
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a1a2e,50:16213e,100:0f3460&height=200&section=header&text=TheMadDoG.tmdg&fontSize=52&fontColor=ffffff&fontAlignY=38&desc=Opérateur,+Renseignement,+Spécialiste&descAlignY=58&descColor=a8b2d8&animation=fadeIn" width="100%"/>

<br/>

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=24&duration=4200&pause=760&color=58A6FF&center=true&vCenter=true&multiline=false&width=580&lines=Armée+de+terre+-+RTSO;11ème+Brigade+Parachutiste+-+2ème+REP;Opérateur+de+Renseignement+du+GCP)](https://git.io/typing-svg)

### 👨‍💻 About Me

![Status](https://img.shields.io/badge/Status-On-purple)
![Status](https://img.shields.io/badge/Niveau-Master-purple)
![Status](https://img.shields.io/badge/Stable-On-purple)

✅ Version “The MadDoG.tmdg”(Dev, rens)

<img src="https://capsule render.vercel.app/api?type=waving&color=0:0f0f0f,50:1a1a1a,100:0f3460&height=200&section=header&text=DOSSIER+OPÉRATIONNEL&fontSize=48&fontColor=ffffff&fontAlignY=38&desc=TheMadDoG.tmdg+-+Infanterie+Parachutiste+%7C+Renseignement+%7C+Ingénierie+Systèmes&descAlignY=58&descColor=a8b2d8&animation=fadeIn" width="100%"/>

<br/>
! Status
! Discipline
! Focus

</div>
</p>

---

# 🪂 SECTION 1
- IDENTITÉ OPÉRATIONNELLE

> Classification : INTERNE / TECHNIQUE
> Diffusion : GITHUB
```rst
IDENTIFIANT : "The MadDoG.tmdg"
ARME : Armée de terre 
UNITÉ : TAP
SPÉCIALITÉ : INFTAP
COMPÉTENCES : Opération du GCM
```

- Profil orienté terrain, analyse, ingénierie, automatisation, avec une approche méthodique, disciplinée, orientée mission.

---

# 📘 SECTION 2 — DOCTRINE & CHAMPS DE COMPÉTENCES

🎯 Renseignement Opérationnel
- Analyse structurée  
- Synthèse rapide  
- Gestion de l’information  
- Transmission fiable  
- Organisation sous contrainte  

💻 Ingénierie & Développement
- Go (API, CLI, data layer, architecture modulaire)
- Shell scripting avancé
- Linux engineering (Mint/Debian)
- Structuration documentaire
- Automatisation & pipelines

🛰️ Corporate / Tech
- GitHub, GitOps, CI/CD
- Documentation professionnelle
- Gestion de projet technique
- Standards reproductibles

---

# 📂 SECTION 3 — DOSSIER TECHNIQUE : PROJET MySelf (v1.3.0)

> Statut : OPÉRATIONNEL  
> Architecture : Collecteur → Data Layer → API REST → Dashboard Web

📌 Logo du système
<img src="web/dashboard/assets/logo.svg" width="140"/>

📌 Aperçu du Dashboard
> (Ouvre web/dashboard/screenshot.html pour générer une capture PNG)

---

# 🧭 SECTION 4 — PROJETS OPÉRATIONNELS

🧠 MySelf
Plateforme d’analyse personnelle.  
Architecture modulaire, robuste, reproductible.

🚚 La Roulotte Solidaire Toulouse
Structuration documentaire, workflows, automatisation.

📡 LearnShell
Projet pédagogique Shell scripting.

🛠 Linux Mint Engineering Edition
Documentation scientifique & architecture système.

---

# 📊 SECTION 5 — INDICATEURS DE PERFORMANCE

| Indicateur | Valeur |
|-----------|--------|
| Missions techniques | 7 |
| Problèmes ONU résolus | 12 |
| Projets livrés | 7 |
| Plateformes | Sitel · Aristote · TRPP39 |
| Domaines | Rens · Dev · Systèmes |

---

# 🎯 SECTION 6 — OBJECTIFS 2026

- MySelf v1.4.0 (Analyse avancée / ML)
- MySelf v2.0.0 (Synchronisation multi‑device)
- LearnShell v2
- Documentation scientifique open‑source
- Structuration avancée de projets solidaires

---

# 📫 SECTION 7 — CONTACT

Disponible pour :  
- projets techniques  
- structuration documentaire  
- automatisation  
- architecture Go  
- ingénierie système  
- missions nécessitant rigueur & efficacité  

---

<div align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f3460,50:16213e,100:000000&height=120&section=footer" width="100%"/>
</div>
`

---

🎖️ Ce que cette version apporte

✔ Style dossier militaire parachutiste
- Sections numérotées  
- Terminologie militaire non‑sensible  
- Ton agressif mais professionnel  
- Structure type “fiche opérationnelle”

✔ Intégration corporate + dev + renseignement
- Ingénierie système  
- Développement Go  
- Analyse structurée  
- Documentation professionnelle  

✔ Intégration logo.svg + screenshot
- Logo intégré en haut  
- Screenshot intégré via fichier HTML  

✔ Conforme aux règles (pas de violence, pas d’armement, pas d’opérations réelles)

---

### 👉 " À Suivre ... "
